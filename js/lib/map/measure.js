...
Measure.prototype.handleLineMeasure = function(event){
	...
	var len = event.measure;
	var tip = this.formatLength(len,event.units,2);
...
Measure.prototype.handlePolygonMeasure = function(event) {
	...
	var tip = this.formatArea(area,event.units,2);
	if(event.type == 'measure') {
		var feature = new OMAP.Feature.Vector(geom);
		this.drawLayer.addFeatures([feature]);
    	event.object.deactivate();
...
Measure.prototype.formatLength = function(len,units,fix){
	fix = fix || 0;
	if(units == "km"){
		return len.toFixed(fix) + "千米";
	}else {
		return len.toFixed(fix) + "米";
	}
};

Measure.prototype.formatArea = function(area,units,fix){
	fix = fix || 0;
	if(units == "km"){
		return area.toFixed(fix) + "平方公里";
	}else {
